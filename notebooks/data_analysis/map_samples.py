...
from data_analysis.filtering import filter_WGS_data, filter_non_WGS_data, find_studies_at_coord, find_samples_per_location 
...
class GeoPlotMap:
    ...
    def __init__(
        self,
        zoom_to_start=2, 
        location_to_start=[13.1339, 16.1039]
    ):
        self.zoom_to_start=zoom_to_start
        self.location_to_start=location_to_start
        self.my_map = folium.Map(
            location=self.location_to_start, zoom_start=self.zoom_to_start, tiles="cartodb positron"
        )
...
def map_samples(data, data_subset=None,zoom_to_start=2,location=[13.1339, 16.1039]):
    ...
    samples_per_country = data.groupby(['Country']).size().reset_index().rename(columns={0:'Samples'})
    ...
    for country in np.unique(data.Country):
        ...
        for _, location in samples_per_location.iterrows():
            #Studies for site
            studies = find_studies_at_coord(country_data, location["Latitude_adm1"],location["Longitude_adm1"])
            #Add marker to country cluster 
            mapper.add_marker_to_cluster(country_cluster, location["Latitude_adm1"], location["Longitude_adm1"], country, int(location.Samples), studies)
    return my_map
